...
class Programs(models.Model):
    ...
    content = models.TextField()
    author = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
    image = models.ImageField(upload_to='Programs_images/')
    created_at = models.DateTimeField(auto_now_add=True)
...
class Chapter(models.Model):
    ...
    chapters_status = models.CharField(max_length=100, choices=CHAPTER_STATUS, blank=True)
...
class Leaderships(models.Model):
    ...
    national_champlain = models.CharField(max_length=100, blank=True, null=True)
    national_champlain_pic = models.ImageField(upload_to='media/leadership/cover', blank=True, null=True)
 
    year = models.CharField(max_length=100, blank=True, null=True)
       
    def __str__(self):
        return f"Leadership - {self.year}" if self.year else "Leadership (no year)"
...
class Membership_From(models.Model): 
    profile_pic = models.ImageField(upload_to='membership/profile_pic/')
    ...
    extra_skill = models.CharField(max_length=100, blank=True)
    level_of_education = models.CharField(max_length=100)

    # FIXED: use ForeignKey to Chapter
    chapter_name = models.ForeignKey("Chapter", on_delete=models.SET_NULL, null=True, blank=True)

    living_with = models.CharField(max_length=50)
    working_or_not = models.CharField(max_length=10, blank=True)
